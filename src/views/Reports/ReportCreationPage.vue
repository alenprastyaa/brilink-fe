  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
    <div class="mx-auto px-3 py-4 sm:px-4 sm:py-8">
      <div class="mb-5 text-center sm:mb-8">
        <div
          class="mb-3 inline-flex h-12 w-12 items-center justify-center rounded-full bg-blue-600 to-indigo-600 sm:mb-4 sm:h-16 sm:w-16">
        ...
        <div class="flex justify-center">
          <div class="flex items-center space-x-1 sm:space-x-4">
            <div v-for="step in totalSteps" :key="step" class="flex items-center">
              ...
              <div v-if="step < totalSteps" class="mx-1 h-1 w-10 transition-all duration-300 sm:mx-2 sm:w-16" :class="step < currentStep
        ...
        <div
          ...
          <form @submit.prevent="handleSubmitReport" @keydown.enter="handleEnterNavigation">
            <div v-show="currentStep === 1" ref="step1Ref" class="p-4 sm:p-8">
              ...
              <div v-if="!isAdmin && stores.length === 0"
              ...
              <div class="grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-6">
                <div class="space-y-2">
                  <FormField label="Pilih Toko" type="select" v-model="selectedStoreId" :options="storeOptions"
                    placeholder="Pilih Toko" required :disabled="isStoreFieldDisabled || stores.length === 0" />
                </div>
                <div class="space-y-2">
                  <FormField label="Tanggal Laporan" type="date" v-model="reportForm.report_date" required />
                </div>
              </div>

              <div class="mt-5 flex justify-end sm:mt-8">
                <button type="button" @click="nextStep" :disabled="!canProceedToStep2 || stores.length === 0"
                  class="w-full rounded-xl bg-blue-600 to-indigo-600 px-6 py-3 font-semibold text-white transition-all duration-200 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:px-8">
                  Lanjutkan
            ...
            <div v-show="currentStep === 2" ref="step2Ref" class="p-4 sm:p-8">
              ...
              <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-6 sm:mb-6">
                <div v-for="bankBalance in reportForm.balances" :key="bankBalance.bank_id"
                  ...
                  <FormField :label="`Saldo ${bankBalance.bank_name}`" type="text" v-model.number="bankBalance.saldo"
                    :placeholder="`Masukkan saldo ${bankBalance.bank_name}`" :min="0" isCurrency />
                </div>
              </div>

              <div class="mb-4 sm:mb-6">
                <FormField label="Jumlah Uang Nitip (Jika ada)" type="text" v-model.number="reportForm.uang_nitip"
                  placeholder="Masukkan jumlah uang nitip dan jangan tulis pada Keterangan" isCurrency :min="0" />
              </div>

              <div class="mb-6 sm:mb-10">
                <label for="keterangan" class="block text-sm font-medium text-gray-700 mb-1">Keterangan</label>
                <textarea v-model="reportForm.keterangan" id="keterangan" name="keterangan" rows="4"
              ...
              <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-between">
                ...
                <button type="button" @click="nextStep"
                  ...
                  class="rounded-xl bg-blue-600 to-indigo-600 px-8 py-3 font-semibold text-white transition-all duration-200 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50">
                  Lanjutkan
                  <svg class="w-5 h-5 ml-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                  </svg>
            ...
            <div v-show="currentStep === 3" ref="step3Ref" class="p-4 sm:p-8">
              ...
              <div class="space-y-4 sm:space-y-6">
                <div class="rounded-xl border border-gray-200 bg-gradient-to-r from-gray-50 to-white p-4 sm:p-6">
                ...
                <div class="rounded-xl border border-green-200 bg-gradient-to-r from-green-50 to-blue-50 p-4 sm:p-6">
                  <h3 class="mb-3 text-base font-semibold text-gray-800 sm:mb-4 sm:text-lg">Detail Saldo</h3>
                  <div class="space-y-3">
                    <div v-for="balance in reportForm.balances.filter((b) => (b.saldo || 0) > 0)" :key="balance.bank_id"
                  ...
                  <div class="mt-10">
                    <label for="keterangan" class="block text-sm font-medium text-gray-700 mb-3">Keterangan</label>
                    <div
