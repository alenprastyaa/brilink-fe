  ...
  <Transition name="modal">
    <div v-if="show"
      ...
      <div
        ...
        <div
          class="sticky top-0 bg-blue-600 to-purple-600 text-white p-6 rounded-t-2xl flex justify-between items-center z-10">
          <div>
            <h3 class="text-2xl font-bold mb-1">{{ report.store_name }}</h3>
            <p class="text-blue-100 text-sm">Laporan Saldo Harian</p>
          </div>
          <button @click="emit('close')" class="text-white hover:text-blue-100 transition-colors duration-200">
        ...
        <div class="p-6 space-y-6">
          ...
          <div>
            ...
            <div class="space-y-3">
              <div v-for="(balance, index) in report.balances_detail" :key="balance.bank_id"
          ...
          <div>
            ...
            <textarea disabled :value="report.keterangan || 'Tidak ada keterangan.'" id="keterangan" name="keterangan"
              rows="4"
              class="block w-full rounded-md border border-gray-300 shadow-sm bg-gray-50 p-3 text-sm text-gray-700 resize-none"></textarea>
          </div>
        </div>
